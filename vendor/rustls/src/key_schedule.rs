...
impl KeySchedule {
    pub fn new(hash: &'static digest::Algorithm) -> KeySchedule {
        ...
        KeySchedule {
            current: hmac::Key::new(hash, &zeroes[..hash.output_len]),
    ...
    pub fn input_secret(&mut self, secret: &[u8]) {
        if self.need_derive_for_extract {
            ...
            self.current = hmac::Key::new(self.hash, &derived);
        }
        self.need_derive_for_extract = true;
        let prk = hmac::sign(&self.current, secret);
        let new = hmac::Key::new(&self.current.digest_algorithm(), prk.as_ref());
    ...
    pub fn sign_verify_data(&self, base_key: &[u8], hs_hash: &[u8]) -> Vec<u8> {
        ...
        let hmac_key = _hkdf_expand_label_vec(&hmac::Key::new(self.hash, base_key),
                                              b"finished",
                                              &[],
                                              self.hash.output_len);

        hmac::sign(&hmac::Key::new(self.hash, &hmac_key), hs_hash)
            .as_ref()
            .to_vec()
    }
    ...
    pub fn derive_next(&self, kind: SecretKind) -> Vec<u8> {
        let base_key = self.current_traffic_secret(kind);
        _hkdf_expand_label_vec(&hmac::Key::new(self.hash, base_key),
                               b"traffic upd",
                               &[],
                               self.hash.output_len)
    }
    ...
    pub fn derive_ticket_psk(&self, rms: &[u8], nonce: &[u8]) -> Vec<u8> {
        _hkdf_expand_label_vec(&hmac::Key::new(self.hash, rms),
                               b"resumption",
                               nonce,
                               self.hash.output_len)
    }
    ...
    pub fn export_keying_material(&self, out: &mut [u8],
        ...
        _hkdf_expand_label(out,
                           &hmac::Key::new(self.hash, &secret[..self.hash.output_len]),
                           b"exporter",
                           &h_context[..self.hash.output_len]);
        Ok(())
    }
...
fn _hkdf_expand_label(output: &mut [u8],
    ...
    let prk = hkdf::Prk::from_key(secret.to_owned());
    let okm = prk.expand(&hkdflabel);
    let _ = okm.fill(output).unwrap();
    //hkdf::expand(secret, &hkdflabel, output)
}

pub fn derive_traffic_key(hash: &'static digest::Algorithm, secret: &[u8], len: usize) -> Vec<u8> {
    _hkdf_expand_label_vec(&hmac::Key::new(hash, secret), b"key", &[], len)
}

pub fn derive_traffic_iv(hash: &'static digest::Algorithm, secret: &[u8], len: usize) -> Vec<u8> {
    _hkdf_expand_label_vec(&hmac::Key::new(hash, secret), b"iv", &[], len)
}
